...
from app.intelligence.causal.rules import ReviewRuleProvider

def wrap_review_rules() -> List[ReasoningRule]:
    provider = ReviewRuleProvider()
    rules = []
...
class ReviewReasoningProvider:
    @property
    def name(self) -> str: return "review"
    def rules(self) -> List[ReasoningRule]: return wrap_review_rules()
