...
from app.intelligence.causal.rules import VelocityRuleProvider

def wrap_velocity_rules() -> List[ReasoningRule]:
    provider = VelocityRuleProvider()
    rules = []
...
class VelocityReasoningProvider:
    @property
    def name(self) -> str: return "velocity"
    def rules(self) -> List[ReasoningRule]: return wrap_velocity_rules()
