...
import os
import asyncio
from evaluation.framework.validators.determinism_validator import DeterminismValidator
...
async def run_determinism():
    ...
    async def run_sync_and_snapshot():
        ...
        h, m_len, e_len, state = DeterminismValidator.snapshot()
        print(f"Snapshot: Hash={h} Measurements={m_len} Evidence={e_len}")
        return state

    print("Running Run A...")
    state_a = await run_sync_and_snapshot()
    
    print("Running Run B...")
    state_b = await run_sync_and_snapshot()
    
    print("Comparing states...")
    passed, report = DeterminismValidator.compare(state_a, state_b)
    print("\n" + report)
    assert passed, "Determinism Validation Failed. See report."
